...
sed -i -e 's|prefix = /usr|prefix = $(PS2SDK)|' Makefile &&
sed -i -e 's|bindir = $(prefix)/bin|bindir = $(PS2SDK)/bin|' Makefile &&
sed -i -e 's|install: all install-bin install-man|install: all install-bin|' Makefile
sed -i -e 's|install -m 755 genromfs|install -m644 genromfs|' Makefile
popd
